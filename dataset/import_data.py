...
ES_APIKEY = os.getenv(
    "ES_APIKEY", decoded.strip())  # Use the decoded password or environment variable
...
def create_index():
    ...
    resp = requests.put(f"{ES_URL}/{INDEX_NAME}",
                        headers=headers, json=mapping, verify=False)
...
def import_csv_to_es():
    with open(CSV_FILE, newline="") as csvfile:
        ...
        for row in reader:
            ...
            results = Parallel(n_jobs=8)(delayed(send_request)(doc)
                                         for _ in range(1))
            import_csv_to_es.pbar.update(1)
            resp = results[0]
            if resp.status_code not in (200, 201):
                print(
                    f"Failed to insert: {doc}, Response: {resp.status_code}, {resp.text}")
